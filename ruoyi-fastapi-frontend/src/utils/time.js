...
/**
 * 请求用户选择夏令时重复时间对应的 UTC 偏移。
 *
 * @param {TimeInputError} error 包含候选偏移的校验异常
...
/**
 * 校验表单时间并构造新的 API 提交数据。
 *
 * @param {Object} record 当前编辑记录
 * @param {string[]} fields 需要转换的真实时刻字段
 * @returns {Promise<Object|null>} 提交数据；校验失败或用户取消时返回 null
 */
export async function serializeTimeFieldsForSubmit(record, fields) {
  try {
    return await resolveTimeFields(record, fields, chooseTimeOffset)
  } catch (error) {
